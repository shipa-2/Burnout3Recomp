...
uint32_t Heap::AllocGuestAligned(size_t size, size_t alignment)
{
    ...
    // Record the raw o1heap pointer so FreeGuest can release the correct block.
    uint32_t guestAddr = g_memory.MapVirtual(reinterpret_cast<void*>(aligned));
    m_sizeMap[guestAddr]   = size;
    m_rawPtrMap[guestAddr] = raw;
    return guestAddr;
}
...
void Heap::FreeGuest(uint32_t guestAddr)
{
    ...
    // Use the raw o1heap pointer (may differ from guestAddr for over-aligned allocs).
    auto rit = m_rawPtrMap.find(guestAddr);
    void* raw = (rit != m_rawPtrMap.end()) ? rit->second : g_memory.Translate(guestAddr);
    if (rit != m_rawPtrMap.end()) m_rawPtrMap.erase(rit);
    o1heapFree(m_heap, raw);
}
